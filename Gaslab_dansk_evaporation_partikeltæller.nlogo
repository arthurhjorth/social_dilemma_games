...
to setup
  clear-all
  ;IBH: fjernet fra interface, default kan ændres her:
  set init-particle-speed 10
  set particle-mass 4

  set original-particle-count Antal-partikler
  set-default-shape particles "circle"
  set-default-shape flashes "plane"
  set max-tick-delta 0.1073
  ;; make floor
  ask patches with [pycor = min-pycor] [set pcolor yellow]
  update-height
  make-particles
  update-variables
...
to-report gravity-acceleration ;IBH: fjernet fra interface, tyngdeaccelerationen kan ændres her:
  ifelse tyngdekraft?
    [ report 9.82 ]
    [ report 0 ]
end
...
to go
  ...
  ask particles
  [ if Kollisioner? [check-for-collision] ]
  if tracer = nobody
  [ set tracer one-of turtles ]
  ifelse Sporing?
  [ ask tracer [ pen-down ] ]
...
to update-variables
  set lost-particles (original-particle-count - count particles)
  set percent-lost-particles (lost-particles / Antal-partikler) * 100
  ...
  set percent-medium (medium / (Antal-partikler - lost-particles)) * 100
  set percent-slow (slow / (Antal-partikler - lost-particles)) * 100
  set percent-fast (fast / (Antal-partikler - lost-particles)) * 100
...
to move  ;; particle procedure
  ...
  let next-patch patch-at xdiff ydiff
  if next-patch = nobody [ die ] ;; AH: this is where they die. We can just lower this...
  if [pycor] of next-patch = max-pycor or [abs pxcor] of next-patch = max-pxcor  [die]
...
to make-particles
  create-particles Antal-partikler
  [
    setup-particle
    random-position
    recolor
...
to update-height
  ask patches with [abs pxcor = 25 and pycor < kop-hoejde] [set pcolor yellow]
  ask patches with [abs pxcor = 25 and pycor > kop-hoejde] [set pcolor black]
end
...
to random-position ;; particle procedure
  setxy (-24 + random-float 48) random-float kop-hoejde + 1
;  setxy random-xcor random-float
end
...
PENS
"Hurtig" 1.0 0 -1893860 true "" "plot count particles with [color = red]"
"Medium" 1.0 0 -11686070 true "" "plot count particles with [color = green]"
"Langsom" 1.0 0 -14979122 true "" "plot count particles with [color = blue]"
